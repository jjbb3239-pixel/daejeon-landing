...
**③ 카페 · 맛집 사진 6장 출처** — 같은 파일 `CREDITS` 의 「카페 · 맛집 사진」이
아직 「출처 확인 중」이다. 명소 4장(대전 100선)과 코스 4장은 2026-08-31 에 채웠다.
...
| 식장산 | 위키미디어 | `photo/sikjangsan-night.jpg` (2026-09-02 교체) |
...
## 0-10. 사진 출처 표기 (2026-08-31)

푸터 「사진 · 콘텐츠 출처」에 사진별 출처를 넣었다.

| 항목 | 출처 |
|---|---|
| 명소 사진 4장 | 「대전 100선」 <https://spot100x100.kr/> |
| 코스 사진 4장 | 아래 표 |
| 카페 · 맛집 사진 6장 | **아직 확인 중** |

코스 사진은 넷 다 출처가 달라서 목록으로 따로 뽑았다.
...
> **공공누리는 출처가 아니라 이용허락 표시다.** 기관명을 앞에 두고 괄호에 넣었다.

### 식장산 사진 교체 (2026-09-02)

원래 쓰던 일출 사진은 첨부 HTML 에서 위키미디어 핫링크로 걸려 있던 것을 내려받은
것이라 **촬영자도 라이선스도 확인이 안 됐다.** 출처가 확실한 사진으로 바꿨다.

| | 바꾸기 전 | 바꾼 뒤 |
|---|---|---|
| 사진 | 정상쉼터 하트 조형물 + 일출 | 정상 팔각정 야경 |
| 파일 | `imports/______.jpg` (700×466) | `imports/photo/sikjangsan-night.jpg` (1400×933) |
| 촬영자 | 미상 | s.h.c (대전광역시 동구 게시물) |
| 라이선스 | 불명 | 공공누리 제1유형 — 출처표시, 상업 이용 O, 변형 O |
| 원문 | — | 동구청 photoNews 88696 → 위키미디어 커먼즈 |

카드 문구가 원래부터 야경이었다(`FINISH · NIGHT VIEW`, 리뷰 "야경이 정말 너무
예뻐요"). 일출 사진이 문구와 어긋나 있던 것도 같이 해결됐다.
`alt` 두 개도 야경 기준으로 고쳤다. 1:1 크롭은 기본값(50%)에서 팔각정과 시내
불빛이 모두 들어와 `object-position` 은 손대지 않았다.
> **유형(제1~4유형)이 확인되면 괄호 안에 같이 적을 것** — 유형에 따라 상업적 이용·변형
> 가능 여부가 달라진다. `SiteFooter.tsx` 의 해당 줄에 주석으로 남겨뒀다.
>
> 「대동 하늘공원」은 사용자가 준 「대전광역시 대전관광」을 정식 명칭인 **대전관광공사**로
> 바꿨다. 다른 기관이면 되돌려야 한다.

`CREDITS` 항목에 `items`(장소↔출처 쌍)와 `link` 를 넣을 수 있게 타입을 넓혔다.

> **리뷰 섹션은 다른 섹션 사진을 재사용한다.** 엑스포다리·이응노(명소), 톨드어스토리·쌍리(카페),
> 대동·식장산(코스)을 그대로 쓴다. 별도로 출처를 적을 사진은 `proof/tomiya.jpg` 와
> `proof/heerak.jpg` 두 장뿐이고, 이 둘은 카페·맛집 묶음에 포함해뒀다.
...
| 31 | 푸터에 사진 출처 표기 (명소 · 코스) | `sections/SiteFooter.tsx` `index.css` |
